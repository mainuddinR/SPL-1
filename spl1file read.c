...
int mainmenu()
{
    ...
    int c,extended,option=1;
    green();
    printf("\n\n\r%26s%c T Y P I N G  T E S T E R%c","",16,17);//26 character space nichi ,
    reset();
    Magenta();
...
\n\r \xaf [OR] Press the number keys to select your choice");
    ...
    setCursorStyle(NOCURSOR);
    DrawMenu(option,menuitem,5);

}
void DrawMenu(int option,char mi[],int n)
{
    char bgcolor[2]={BLACK,LIGHTGRAY};
    box(28,10,25,n*2+2,YELLOW,BLACK);
    puts(mi[n]);
    int i;
    for(i=0;i<n;i++){
        textbackground(bgcolor[(option==(i+1))]);
        printf(mi[i]);
    }
}
int box(int x,int y,int height,int width,int fc,int bc)
{
    int i;
    x-=1;
    y-=1;
